<?php
/**
 * Froststone Registration Portal - Footer Template (WotLK Theme)
 **/
?>
<div class="section-divider"></div>
<footer class="site-footer">
    <div class="footer-inner">
        <div class="footer-left">Froststone &middot; For the Horde &middot; For the Alliance</div>
...
/* Frost particle canvas (WotLK icy blue theme) */
(function() {
    ...
    Particle.prototype.draw = function() {
        ...
        ctx.fillStyle = 'rgba(124, 185, 232, ' + this.currentOpacity + ')';
        ctx.fill();
        if (this.size > 1.5) {
            ctx.beginPath();
            ctx.arc(this.x, this.y, this.size * 3, 0, Math.PI * 2);
            ctx.fillStyle = 'rgba(124, 185, 232, ' + (this.currentOpacity * 0.12) + ')';
            ctx.fill();
        }
    };
    ...
    function drawRune(cx, cy, radius, angle) {
        ...
        ctx.strokeStyle = 'rgba(124, 185, 232, 0.06)';
        ctx.lineWidth = 1.5;
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(0, 0, radius * 0.72, 0, Math.PI * 2);
        ctx.strokeStyle = 'rgba(124, 185, 232, 0.04)';
        ctx.lineWidth = 1;
        ctx.stroke();
        for (var i = 0; i < 8; i++) {
            var a = (Math.PI * 2 / 8) * i;
            ...
            ctx.strokeStyle = 'rgba(124, 185, 232, 0.05)';
            ctx.lineWidth = 1;
            ctx.stroke();
        }
        for (var j = 0; j < 4; j++) {
            ...
            ctx.closePath();
            ctx.fillStyle = 'rgba(124, 185, 232, 0.08)';
            ctx.fill();
            ctx.restore();
        }
        ctx.restore();
